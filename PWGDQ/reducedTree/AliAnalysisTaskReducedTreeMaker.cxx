...
Bool_t AliAnalysisTaskReducedTreeMaker::CheckPDGcode(AliMCEvent* event, Int_t ipart, AliSignalMC* mcSignal) {
   ...
   for(UInt_t ig=0; ig<mcSignal->GetNGenerations(); ++ig) {
      ...
      currentGenerationParticle = ((currentGenerationParticle && (currentGenerationLabel != -1)) ? event->GetTrack(currentGenerationLabel) : 0x0);
   }
   return kTRUE;
}
...
Bool_t AliAnalysisTaskReducedTreeMaker::CheckParticleSourceAOD(AliMCEvent* event, Int_t ipart, AliSignalMC* mcSignal) {
   ...
   for(UInt_t ig=0; ig<mcSignal->GetNGenerations(); ++ig) {
      ...
      if(mcSignal->CheckSourceBit(0,ig, AliSignalMC::kFromSubsidiaryEvent)) {
         if(mcSignal->GetSourceExclude(0,ig,AliSignalMC::kFromSubsidiaryEvent) != currentGenerationParticle->IsFromSubsidiaryEvent())//event->IsFromSubsidiaryEvent(currentGenerationLabel))
            decision |= (UInt_t(1) << AliSignalMC::kFromSubsidiaryEvent);
      }
      if(mcSignal->CheckSourceBit(0,ig, AliSignalMC::kRadiativeDecay)) {
         if(mcSignal->GetSourceExclude(0,ig,AliSignalMC::kRadiativeDecay) != (currentGenerationParticle->GetNDaughters()>2))
...
void AliAnalysisTaskReducedTreeMaker::FillMCTruthInfo(Bool_t isAOD)
{
   ...
   for(Int_t i=0; i<event->GetNumberOfTracks(); ++i) {
      ...
      if ((isAOD && particle->IsFromSubsidiaryEvent()) || (!isAOD && event->IsFromSubsidiaryEvent(i))) break;
      ...
      if(eventInfo){
        if(particle->IsPhysicalPrimary() && particle->Charge()){
          ...
          AliVParticle* mother = (particle->GetMother() != -1) ? event->GetTrack(particle->GetMother()) : nullptr;
      ...
      if(mother) {
         ...
         AliVParticle* grandmother = (mother->GetMother() != -1) ? event->GetTrack(mother->GetMother()) : nullptr;
         if(grandmother) {
            trackInfo->fMCLabels[2] = grandmother->GetLabel();
            trackInfo->fMCPdg[2] = grandmother->PdgCode();
            
            AliVParticle* grandgrandmother = (grandmother->GetMother() != -1) ? event->GetTrack(grandmother->GetMother()) : nullptr;
            if(grandgrandmother) {
